 ...
 * users to enter a new student message
 *
 */
...
{   
    public function cancel($key = null)
    {
        ...
        $this->setRedirect(
            "index.php?option=com_student&view=students",
            JText::_(ADD_CANCELLED)
		);
    }
    
    /*
     * Function handing the save for adding a new student record
    ...
    public function save($key = null, $urlVar = null)
    {
		...
		if (!JFactory::getUser()->authorise( "core.create", "com_student"))
		{
		...
		// notify the administrator that a new student message has been added on the front end
		...
		$mailer->setSubject("New student message added by " . $current_username);
		$mailer->setBody("New student name is " . $validData['fullname']);
		try 
		{
			$mailer->send(); 
		}
		...
		$this->setRedirect(
				"index.php?option=com_student&view=students",
				JText::_('ADD NEW STUDENT SUCCESSFUL')
				);
	...
	public function edit($data = array(), $key = 'id')
	{
		$id = isset( $data[ $key ] ) ? $data[ $key ] : 0;
		if( !empty( $id ) )
		{
			return JFactory::getUser()->authorise( "core.edit", "com_student.student." . $id );
		}
	}
}